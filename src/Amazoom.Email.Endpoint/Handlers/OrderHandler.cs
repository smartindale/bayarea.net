using Amazoom.Email.Contracts.Events;
...
namespace Amazoom.Email.Endpoint.Handlers
{
    public class OrderHandler : IHandleMessages<IPlacedAnOrder>
    {
        ...
        {
            _log.WarnFormat("Sending email for order {0}", message.OrderId);
            _bus.Publish<ISentAnOrderNotification>(e =>
            {
